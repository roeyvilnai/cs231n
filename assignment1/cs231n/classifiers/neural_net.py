...
class TwoLayerNet(object):
  ...
  def loss(self, X, y=None, reg=0.0):
    ...
    right_class_scores = scores_exp[np.arange(len(scores_exp)), y]
    ...
    dscores = scores_prob.copy()
    dscores[range(N), y] -= 1
    dscores /= N

    dW2 = hidden_layer.T.dot(dscores) + reg * W2
    grads['W2'] = dW2
  ...
  def predict(self, X):
    ...
    b1 = b1[:, np.newaxis]
    b2 = b2[:, np.newaxis]
    h = np.maximum(0, np.dot(W1.T, X.T) + b1)
    scores = np.dot(W2.T, h) + b2
